...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap):
    houghLines = cv2.HoughLinesP(img, rho, theta, threshold, np.array([]), minLineLength=min_line_len,
...
def draw_lanes(image, showIntermediateSteps=False, imageName=""):
    ...
    vertices = np.array(
        [[0, imgHeight], [imgWidth * 0.45, yTopMask], [imgWidth * 0.55, yTopMask], [imgWidth, imgHeight]], np.int32)
    ...
    houghLines = cv2.HoughLinesP(maskedImage, rho, theta, threshold, np.array([]), minLineLength=min_line_length,
                                 maxLineGap=max_line_gap)
...
draw_lanes_image("test_images/solidWhiteCurve.jpg")
...
output = 'test_videos_output/processed_video_2.mp4'
clip1 = VideoFileClip("test_videos/reto2.mp4")

reset_params_for_averaging()
clip_out = clip1.image_transform(draw_lanes)  # NOTE: this function expects color images!!
